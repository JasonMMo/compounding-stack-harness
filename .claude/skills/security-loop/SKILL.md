...
| 4 | **취약점 클래스** | 대상별 재현 점검: SQL injection (파라미터 바인딩 여부), 인증·인가 우회, 경로 탐색, XSS (출력 이스케이프), 안전하지 않은 역직렬화, 의존성 CVE. **★RLS/멀티테넌트 격리 보유 버티컬은 live(또는 `pytest -m postgres`) cross-tenant 격리 단언이 PASS 의 hard prerequisite — mock DB 단위테스트는 RLS 미실증(SET ROLE·FORCE RLS·set_config 를 실행 안 함). 2-주체 disjoint 집합·zero-overlap 단언 없이 GO 금지** (Growth-95: app_service 슈퍼유저 접속으로 RLS 우회된 BLOCK 누수가 mock 통과·인프라게이트 이연으로 라이브까지 생존) | 각 클래스 PASS/finding + 라인 근거. RLS 버티컬은 live 격리 단언 PASS 첨부 |
...
- "안전해 보임" 판정 (재현 명령 없음) / 보안 BLOCK 을 시간 압박으로 약화 / CAVEAT 의 해소 조건 누락 / live-검증 안 된 보안 능력을 인도 문서에 약속 / 시크릿 발견을 조용히 수정 (에스컬레이션 의무 — git history rotate 필요) / **RLS·테넌트 격리를 mock DB 단위테스트만으로 PASS 판정 (Growth-95 — live cross-tenant 단언 없이는 격리 미실증)**
