...
RLS CONTRACT:
  ...
  - app_user role:    RLS enforced — rls_session() issues SET LOCAL ROLE
    ...
    = '<user_uuid>' so RLS policies (defined TO app_user) are enforced.
  ...
  - Missing user_id → 0 rows returned (fail-safe, not an exception).
...
Usage:
    ...
    # For user-scoped reads (RLS enforced):
    async with pool.connection() as conn:
        async with rls_session(conn, user_id):
...
async def rls_session(
    ...
    Wraps the body in a transaction and issues (in order):
        ...
        SELECT set_config('app.current_user_id', '<validated_user_uuid>', true)
    ...
    Raises:
        RLSSessionError: if user_id is not a valid UUID.
    """
    safe_id = _validate_uuid(user_id, "user_id")
    ...
    async with conn.transaction():
        ...
        logger.debug("RLS session opened (role=app_user) for user_id=%s", safe_id)
        yield
