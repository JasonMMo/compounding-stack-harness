...
# These fixtures require LEGAL_RAG_DB_DSN_POSTGRES to be set.
# When unset they skip — keeping the unit suite green on dev boxes with no DB.
...
def _require_pg_dsn() -> str:
    """Return LEGAL_RAG_DB_DSN_POSTGRES or skip the test if unset."""
    dsn = os.environ.get("LEGAL_RAG_DB_DSN_POSTGRES", "").strip()
    if not dsn:
        pytest.skip(
            "LEGAL_RAG_DB_DSN_POSTGRES not set — postgres integration skipped"
        )
    return dsn
...
async def pg_conn():
    ...
    Skips automatically when LEGAL_RAG_DB_DSN_POSTGRES is unset or
