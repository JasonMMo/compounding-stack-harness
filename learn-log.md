...
- **잔여(open)**: ① **flux-demo LIVE 배포**(Coolify, 6번째 데모 카드·레지스트리) 미완. ② pull-quote-wall 등 items[] 기반 variant가 catalog testimonial copy_slots.required(quote/author_name)를 강제당해 profile에 중복 기입 — copy_slots를 variant-aware optional로 완화 검토. ③ A3(Event)·A5(Mobile App) 아키타입.
- **상세**: [cdo ledger](docs/learn-logs/cdo.md) · [engineer ledger](docs/learn-logs/engineer.md)
- **결정 메모리**: [[marketing-site-track]] · [[infra-stack]]
