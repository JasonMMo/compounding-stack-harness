...
- **잔여**: ~~Phase2 가드5종~~(WP-2 완료: G-16~G-20 + 카탈로그) · ~~Phase3 복제본 CLI~~(WP-3 완료: build_replica.py + Style Dictionary 평가) **종결**. **잔여 = Phase1/WP-4 파일럿**(claude-design cloud craft 1종 도착 필요 — landing-astro 테마 craft→sync→정규화→variant→라이브 측정→design-loop SKILL 박제, blocked). legal 고객엔 BAA beta졸업 재검증 전 PII 금지.
