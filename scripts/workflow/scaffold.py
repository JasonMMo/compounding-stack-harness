...
def emit_ddl(
    ...
    """Call render.py for each entity and concatenate output → out_dir/ddl/<dialect>.sql.

    Uses render.py --entity per key to leverage its topological + FK logic.
    Returns the path to the written SQL file.
    """
    render_script = REPO_ROOT / "presets" / "ddl" / "render.py"

    # Collect DDL for all entities (render.py handles topological order per call)
    # For a coherent multi-entity SQL file we call once per domain subset, then
    # deduplicate. Simplest correct approach: call render.py with no filter
    # to get full topo-sorted output, then filter the output lines we care about.
    # However render.py has no --entities (plural) flag. Instead we call it
    # with the full catalog and accept all tables — for Phase 1 this is sufficient
    # since the SQL is a derived artifact (out/ is gitignored).
    #
    # For a minimal subset: call once per entity with --entity flag.
    # Topological ordering within a subset is handled by render.py per call.
    # We prepend a single header then concatenate body blocks.
    ...
    for key in entity_keys:
        result = subprocess.run(
            [sys.executable, str(render_script), "--dialect", dialect, "--entity", key],
            capture_output=True,
            text=True,
        )
        if result.returncode != 0:
            print(f"render.py error for entity '{key}': {result.stderr.strip()}", file=sys.stderr)
            raise RuntimeError(f"DDL render failed for entity '{key}'")
        # Strip the per-call header lines (lines starting with --)
        body_lines = [
    ...
    sql_path.write_text("\n\n".join(filter(None, all_sql_parts)), encoding="utf-8")
    return sql_path
